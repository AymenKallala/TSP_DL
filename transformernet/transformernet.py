...
from utils import check_finished_routes
...
def compute_tour_length(x, tour):
    ...
    with torch.no_grad():
        for i in range(1, nb_nodes):
            ...
            L += (
                torch.sum((current_cities - previous_cities) ** 2, dim=1) ** 0.5
            )  # dist(current, previous node)
            previous_cities = current_cities
        L += (
            torch.sum((current_cities - first_cities) ** 2, dim=1) ** 0.5
        )  # dist(last, first node)
    return L
...
class TSP_net(nn.Module):
    ...
    def forward(self, x, starting_mask=None):
        ...
        tours = []
        # list that will contain Long tensors of shape (bsz,nb_nodes) that gives the probs of the cities chosen at time t
        probs_next_city = []
