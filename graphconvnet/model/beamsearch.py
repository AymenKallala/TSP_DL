...
class Beamsearch(object):
    ...
    def __init__(
        ...
        if random_start == True:
            ...
            self.start_nodes = torch.randint(
                0, num_nodes, (batch_size, beam_size)
            ).type(self.dtypeLong)
    ...
    def advance(self, trans_probs):
        ...
        if len(self.prev_Ks) > 0:
            ...
            if self.probs_type == "raw":
                beam_lk[:, 1:] = torch.zeros(beam_lk[:, 1:].size()).type(
                    self.dtypeFloat
                )
            elif self.probs_type == "logits":
                beam_lk[:, 1:] = -1e20 * torch.ones(beam_lk[:, 1:].size()).type(
                    self.dtypeFloat
                )
        # Multiply by mask
        beam_lk = beam_lk * self.mask
        beam_lk = beam_lk.view(
            self.batch_size, -1
        ...
        # Re-index mask
        perm_mask = prev_k.unsqueeze(2).expand_as(
            self.mask
        )  # (batch_size, beam_size, num_nodes)
